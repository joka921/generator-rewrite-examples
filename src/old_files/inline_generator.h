...
class inline_generator_iterator {
 ...
 public:
  ...
  explicit inline_generator_iterator(handle_type* handle) noexcept : m_handle(handle) {}

  friend bool operator==(const inline_generator_iterator& it, generator_sentinel) noexcept {
    return !it.m_handle || it.m_handle->done();
  }

  friend bool operator!=(const inline_generator_iterator& it, generator_sentinel s) noexcept {
    return !(it == s);
  }

  friend bool operator==(generator_sentinel s, const inline_generator_iterator& it) noexcept {
    return (it == s);
  }
...
class [[nodiscard]] inline_generator {
 public:
  ...
  explicit inline_generator(CoroFrame&& frame) : m_handle(std::move(frame)) {}
