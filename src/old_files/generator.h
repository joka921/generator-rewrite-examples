...
// A simple generator class. It is templated on the used `stackless_coroutine_handle` type, so we
// can either use it with `std::coroutine_handle`, or with our own.
template <typename T, template <typename...> typename GeneratorHandle>
class generator;
...
template <typename T, template <typename...> typename GeneratorHandle, bool ConstDummy = false>
...
class [[nodiscard]] generator {
 public:
  ...
  using iterator = detail::generator_iterator<T, GeneratorHandle, false>;
  ...
  explicit generator(GeneratorHandle<promise_type> coroutine) noexcept : m_coroutine(coroutine) {}
...
namespace detail {
template <typename T, template <typename...> typename H>
generator<T, H> generator_promise<T, H>::get_return_object() noexcept {
