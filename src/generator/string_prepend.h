 ...
 *   generator<std::string, std::coroutine_handle> prepend(auto&& rangeOfStrings, std::string
 * prefix) { for (const auto& s : rangeOfStrings) { co_yield(prefix + s);
 *     }
 *   }
 *
 */
...
heap_generator<std::string> string_prepend(RangeOfStrings&& rangeOfStrings, std::string prefix) {
  ...
  struct CoroFrame : stackless_coro_crtp<CoroFrame, promise_type, true> {
    ...
    // We will have local variables for `it` and `end`, as well as for a temporary `std::string`
    // object during the `co_yield` exression.
    ...
    void destroySuspendedCoro(size_t suspendIdx_) {
      switch (suspendIdx_) {
        ...
        case 1:
          ...
          tmp_.destroy();
        case 2:
          // Nothing to destroy at the `co_return` point, as there are no local variables in scope
          // at that point.
          return;
      }
    }
