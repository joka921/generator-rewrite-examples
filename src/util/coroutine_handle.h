...
struct stackless_coroutine_handle;
template <>
struct stackless_coroutine_handle<void>;
...
// a default-constructed stackless_coroutine_handle<void> (ptr=nullptr) means "no more symmetric
// transfers".
struct HandleFrame {
...
// Common base class for `stackless_coroutine_handle<void>`  and
// stackless_coroutine_handle<some_promise_type>` below.
namespace detail {
struct stackless_handle_base {
  ...
  stackless_handle_base& operator=(std::nullptr_t) noexcept {
    ptr = nullptr;
    return *this;
  }

  // Trampoline: keep resuming returned handles until we get a null handle
  // (no more symmetric transfers). Defined out-of-line after stackless_coroutine_handle<void> is
  // complete.
  void resume();
...
// A replacement for `std::coroutine_handle`. Consists of a single pointer to the `HandleFrame` from
// above, to which it delegates all the member functions.
template <typename Promise>
struct stackless_coroutine_handle : public detail::stackless_handle_base {
  ...
  //  Assumes that in our final coroutine frame the promise object will be declared directly after
  //  the
  // `HandleFrame` above.
  ...
  static stackless_coroutine_handle from_promise(Promise& p) {
    auto* ptr = reinterpret_cast<HandleFrame*>(reinterpret_cast<char*>(&p) - promise_offset);
    return stackless_coroutine_handle{ptr};
  }

  const Promise& promise() const {
    return *reinterpret_cast<const Promise*>(reinterpret_cast<const char*>(ptr) + promise_offset);
  }
...
// Equivalent of std::noop_coroutine() — returns a stackless_coroutine_handle<void> that does
// nothing on resume/destroy.
namespace detail {
...
inline stackless_coroutine_handle<void> noop_coroutine() {
  static HandleFrame noop_frame{&detail::noop_resume, &detail::noop_destroy};
  return stackless_coroutine_handle<void>{&noop_frame};
}

// A stackful coroutine handle that is templated on the frame of a certain coroutine and stores this
// frame by value. Note: It is also possible to template this class on a reference-type, then only a
// (still strongly typed) reference to the frame will be stored.
...
struct stackful_coroutine_handle {
  ...
  // of the coroutine type (e.g. a generator) that owns the handle (same as for ordinary stackless
  // coroutines).
