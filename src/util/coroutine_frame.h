...
struct stackless_coro_crtp {
    // Required by the `stackless_coroutine_handle` class: the `promise` has to be declared directly after the `HandleFrame`.
    HandleFrame frame_;
    PromiseType promise_;

    static void CHECK() {
        static_assert(offsetof(stackless_coro_crtp, promise_) -
                      offsetof(stackless_coro_crtp, frame_) ==
    ...
    // stores the currently active try-block, and `CO_NO_TRY_BLOCK` means that there is no active try-block,
    ...
    [[no_unique_address]] coro_storage<decltype(std::declval<PromiseType&>().initial_suspend())&, true> initial_awaiter_;
    ...
    bool done() const
    {
        return frame_.resumeFunc == nullptr;
    }
    void setDone()
    {
        frame_.resumeFunc = nullptr;
    }

    // Helper function to cast the `void*` which is obtained from the `stackless_coroutine_handle` into a `Derived*`.
    ...
    void destroy() {
        ...
        // Destroy and delete the `Derived` object itself (it was allocated on the heap via the `ramp()` below.
        self->~Derived();
    ...
    static stackless_coroutine_handle<void> doStep(void* ptrToDerived) noexcept(isNoexcept) {
        ...
        } else
        {
            ...
            } catch (...)
            {
                ...
                return derived.handleException();
            }
        }
    }
    ...
    static auto ramp(CoroArgs&&... coroArgs) {
        ...
        void *coroMem = coro_detail::promise_allocate<PromiseType>(sizeof(Derived), std::forward<CoroArgs>(coroArgs)...);
    ...
    // Returns stackless_coroutine_handle<void> for the trampoline: if the catch handler does symmetric transfer,
