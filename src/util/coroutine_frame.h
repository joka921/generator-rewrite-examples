...
struct stackless_coro_crtp
{
    ...
    // Check that the pointer magic in the `stackless_coroutine_handle` is compatible with the layout of this class
    void CHECK()
    {
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Winvalid-offsetof"
        // The offsetof is technically fishy, as the `PromiseType` might not be standard layout, but we additionally
        // also add a runtime check.
        ...
        if (reinterpret_cast<std::uintptr_t>(&promise_) - reinterpret_cast<std::uintptr_t>(&frame_) !=
            stackless_coroutine_handle<PromiseType>::promise_offset)
        {
            std::terminate();
        }
