...
// NOTE/Caveat: The actual TYPE of the object is currently not stored, so a `const int` and `const
// int& = <someTemporary>` cannot be told apart e.g. for `decltype` expressions.
...
struct coro_storage {
  ...
  using Storage =
      std::conditional_t<isOwning, std::decay_t<Ref>, std::add_pointer_t<std::decay_t<Ref>>>;
  alignas(Storage) char buffer[sizeof(Storage)];

  // Proxy type to obtain a reference to the stored value. The correct way to obtain a reference on
  // a `coro_storage c` is to call `c.get().ref_`. The proxy type gives us the correct behavior for
  // `rvalue` references, because without the proxy object, when using an accessor function, they
  // would behave like being wrapped in `std::move`.
  struct Proxy {
    Ref ref_;
  };
...
// the buffer.
// Examples:
// `int a = 4; coro_storage<int&, false> storage; CO_STORAGE_CONSTRUCT(storage, (a))` stores a
// pointer. `coro_storage<std::vector<int>&&, true> storage; CO_STORAGE_CONSTRUCT(storage, {3, 4,
// c})` uses brace initialization
//  into the buffer.
